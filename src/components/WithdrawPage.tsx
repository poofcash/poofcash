...
const WithdrawPage = () => {
    ...
    const withdrawHandler = async () => {
        ...
        try {
            ...
            let { proof, args } = await generateProof({ deposit, recipient, relayerAddress, refund, tornado });
            ...
            try {
                const relay = await axios.post(RELAYER_URL + '/relay', { contract: tornadoAddress, proof, args });
            ...
            setState({ ...state, loading: false, proofGenerated: false, txSent: true, noteWithdraw: '', error: false });
        } catch (e) {
            console.log('ERROR: Withdraw transaction not sent', e);
            setState({ ...state, loading: false, proofGenerated: false, txSent: false, error: true });
        }
    };
    ...
    return (
        <div className="withdraw-wrapper">
            <label className="withdraw-note-label">
                <b>Your Note:</b>
                <input name="note" type="text" value={state.noteWithdraw} onChange={handleChange} />
            </label>
            <br />
            <label className="btc-recipient-label">
                <b>Recipient's Ethereum Address:</b>
                <input name="ethRecipientAddress" type="text" value={state.ethAddress} onChange={handleChange} />
            </label>
